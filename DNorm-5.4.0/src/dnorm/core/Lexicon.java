...
public class Lexicon {
    ...
    public Lexicon(DiseaseNameAnalyzer analyzer, Dictionary dict) {
        ...
        analyzedNamesToConceptIds = new HashMap<>();
    ...
    public void addConcept(String conceptId, String name, boolean isPreferred) {
        ...
        try {
            ...
            doc.add(new Field("conceptId", conceptId, fieldtype1));

            FieldType fieldtype2 = new FieldType();
            fieldtype2.setStored(true);
            fieldtype2.setTokenized(true);
            doc.add(new Field("name", name, fieldtype2));
    ...
    public void prepare() {
        try {
            if (writer != null) {
                ...
                dict.freeze();
                System.err.println("Lucene done");
                ...
                System.err.println("Token weights calculated");
                vectorsToConceptIds = new HashMap<>();
                conceptIdToVectors = new HashMap<>();
                ...
                for (AnalyzedName analyzedName : analyzedNamesToConceptIds.keySet()) {
                    ...
                    Set<String> conceptIds_ = analyzedNamesToConceptIds.get(analyzedName);
                    // t1 += System.currentTimeMillis();
                    ...
                    vectorsToConceptIds.put(vector, conceptIds_);
                    // t4 += System.currentTimeMillis();
                    ...
                    // }
                    for (String conceptId : conceptIds_) {
...
//        }
        ...
        frequency += reader.docFreq(new Term("name", token));
...
//            }
    ...
    public Set<DoubleMatrix2D> getNameVectorsForConcept(String conceptId) {
        try {
            TermQuery query = new TermQuery(new Term("conceptId", conceptId));
            TopDocs topDocs = searcher.search(query, 1000); // TODO Get all hits
            Set<DoubleMatrix2D> vectors = new HashSet<>();
            ScoreDoc[] hits = topDocs.scoreDocs;
            for (int i = 0; i < hits.length; i++) {
                Document doc = searcher.doc(hits[i].doc);
    ...
    private class ExactLookup implements Lookup {
        ...
        public ScoreDoc[] lookup(String lookupText) {
            try {
                ...
                while (tokenStream.incrementToken()) {
                    String token = cattr.toString();
                    builder.add(new TermQuery(new Term("name", token)), Occur.SHOULD);
                }
                TopDocs topDocs = searcher.search(builder.build(), maxMatches);
                ScoreDoc[] hits = topDocs.scoreDocs;
                return hits;
            } catch (IOException e) {
                // TODO Improve exception handling
    ...
    private class FuzzyLookup implements Lookup {
        ...
        public ScoreDoc[] lookup(String lookupText) {
            BooleanQuery.Builder builder = new BooleanQuery.Builder();
            try (TokenStream tokenStream = analyzer.tokenStream("name", new StringReader(lookupText))) {
                CharTermAttribute cattr = tokenStream.addAttribute(CharTermAttribute.class);
                tokenStream.reset();
                while (tokenStream.incrementToken()) {
                    String token = cattr.toString();
                    ...
                    System.out.println("****************************");
                    System.out.println("****************************");
                    builder.add(new FuzzyQuery(new Term("name", token), (int) minimumSimilarity, prefixLength, maxExpansions, false), Occur.SHOULD);
                }
                TopDocs topDocs = searcher.search(builder.build(), maxMatches);
                ScoreDoc[] hits = topDocs.scoreDocs;
                return hits;
