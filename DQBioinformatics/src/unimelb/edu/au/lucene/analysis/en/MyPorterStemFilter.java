 ...
 * class MyAnalyzer extends Analyzer { {@literal @Override} protected
 * TokenStreamComponents createComponents(String fieldName) { Tokenizer source =
 * new LowerCaseTokenizer(version, reader); return new
 * TokenStreamComponents(source, new PorterStemFilter(source)); } }
 * </PRE>
 * <p>
 * Note: This filter is aware of the {@link KeywordAttribute}. To prevent
 * certain terms from being passed to the stemmer
 * {@link KeywordAttribute#isKeyword()} should be set to <code>true</code> in a
 * previous {@link TokenStream}.
 *
 * Note: For including the original term as well as the stemmed version, see
 * {@link org.apache.lucene.analysis.miscellaneous.KeywordRepeatFilterFactory}
 * </p>
 */
public final class MyPorterStemFilter extends TokenFilter {

    private final MyPorterStemmer stemmer = new MyPorterStemmer();
    private final CharTermAttribute termAtt = addAttribute(CharTermAttribute.class);
    private final KeywordAttribute keywordAttr = addAttribute(KeywordAttribute.class);

    public MyPorterStemFilter(TokenStream in) {
        super(in);
    }
    ...
    public final boolean incrementToken() throws IOException {
        ...
        if ((!keywordAttr.isKeyword()) && stemmer.stem(termAtt.buffer(), 0, termAtt.length()) && !termAtt.toString().matches(".*\\d+.*")) {// modifications of reda
