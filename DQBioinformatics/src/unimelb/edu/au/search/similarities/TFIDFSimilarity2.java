 ...
 *
 * <p>
 * The following describes how Lucene scoring evolves from underlying
 * information retrieval models to (efficient) implementation. We first brief on
 * <i>VSM Score</i>, then derive from it <i>Lucene's Conceptual Scoring
 * Formula</i>, from which, finally, evolves <i>Lucene's Practical Scoring
 * Function</i>
 ...
 * Vector Space Model (VSM) of Information Retrieval</a> - documents "approved"
 * by BM are scored by VSM.
 *
 * <p>
 * In VSM, documents and queries are represented as weighted vectors in a
 * multi-dimensional space, where each distinct index term is a dimension, and
 * weights are
 * <a href="http://en.wikipedia.org/wiki/Tfidf">Tf-idf</a> values.
 *
 * <p>
 * VSM does not require weights to be <i>Tf-idf</i> values, but <i>Tf-idf</i>
 * values are believed to produce search results of high quality, and so Lucene
 * is using <i>Tf-idf</i>.
 * <i>Tf</i> and <i>Idf</i> are described in more detail below, but for now, for
 * completion, let's just say that for given term <i>t</i> and document (or
 * query) <i>x</i>,
 * <i>Tf(t,x)</i> varies with the number of occurrences of term <i>t</i> in
 * <i>x</i>
 * (when one increases so does the other) and
 * <i>idf(t)</i> similarly varies with the inverse of the number of index
 * documents containing term <i>t</i>.
 *
 * <p>
 * <i>VSM score</i> of document <i>d</i> for query <i>q</i> is the
 ...
 * of the weighted vectors, and <i>|V(q)|</i> and <i>|V(d)|</i> are their
 * <a href="http://en.wikipedia.org/wiki/Euclidean_norm#Euclidean_norm">Euclidean
 * norms</a>.
 *
 * <p>
 * Note: the above equation can be viewed as the dot product of the normalized
 * weighted vectors, in the sense that dividing
 * <i>V(q)</i> by its euclidean norm is normalizing it to a unit vector.
 *
 * <p>
 * Lucene refines <i>VSM score</i> for both search quality and usability:
 * <ul>
 * <li>Normalizing <i>V(d)</i> to the unit vector is known to be problematic in
 ...
 * factor is used, which normalizes to a vector equal to or larger than the unit
 * vector: <i>doc-len-norm(d)</i>.
 * </li>
 *
 * <li>At indexing, users can specify that certain documents are more important
 * than others, by assigning a document boost. For this, the score of each
 * document is also multiplied by its boost value
 * <i>doc-boost(d)</i>.
 * </li>
 *
 * <li>Lucene is field based, hence each query term applies to a single field,
 * document length normalization is by the length of the certain field, and in
 * addition to document boost there are also document fields boosts.
 * </li>
 *
 * <li>The same field can be added to a document during indexing several times,
 * and so the boost of that field is the multiplication of the boosts of the
 * separate additions (or parts) of that field within the document.
 * </li>
 *
 * <li>At search time users can specify boosts to each query, sub-query, and
 * each query term, hence the contribution of a query term to the score of a
 * document is multiplied by the boost of that query term <i>query-boost(q)</i>.
 * </li>
 *
 * <li>A document may match a multi term query without containing all the terms
 * of that query (this is correct for some of the queries), and users can
 * further reward documents matching more query terms through a coordination
 * factor, which is usually larger when more terms are matched:
 * <i>coord-factor(q,d)</i>.
 * </li>
 * </ul>
 *
 * <p>
 * Under the simplifying assumption of a single field in the index, we get
 * <i>Lucene's Conceptual scoring formula</i>:
 *
 * <br>&nbsp;<br>
 * <table cellpadding="2" cellspacing="2" border="0" style="width:auto; margin-left:auto; margin-right:auto" summary="formatting only">
 * <tr><td>
 ...
 *
 * <p>
 * The conceptual formula is a simplification in the sense that (1) terms and
 ...
 *
 * <p>
 * For efficient score computation some scoring components are computed and
 * aggregated in advance:
 *
 * <ul>
 * <li><i>Query-boost</i> for the query (actually for each query term) is known
 * when search starts.
 * </li>
 *
 * <li>Query Euclidean norm <i>|V(q)|</i> can be computed when search starts, as
 * it is independent of the document being scored. From search optimization
 * perspective, it is a valid question why bother to normalize the query at all,
 * because all scored documents will be multiplied by the same <i>|V(q)|</i>,
 * and hence documents ranks (their order by score) will not be affected by this
 * normalization. There are two good reasons to keep this normalization:
 * <ul>
 * <li>Recall that
 * <a href="http://en.wikipedia.org/wiki/Cosine_similarity">
 * Cosine Similarity</a> can be used find how similar two documents are. One can
 * use Lucene for e.g. clustering, and use a document as a query to compute its
 * similarity to other documents. In this use case it is important that the
 * score of document <i>d3</i>
 * for query <i>d1</i> is comparable to the score of document <i>d3</i>
 * for query <i>d2</i>. In other words, scores of a document for two distinct
 * queries should be comparable. There are other applications that may require
 * this. And this is exactly what normalizing the query vector <i>V(q)</i>
 * provides: comparability (to a certain extent) of two or more queries.
 * </li>
 *
 * <li>Applying query normalization on the scores helps to keep the scores
 * around the unit vector, hence preventing loss of score data because of
 * floating point precision limitations.
 * </li>
 * </ul>
 * </li>
 *
 * <li>Document length norm <i>doc-len-norm(d)</i> and document boost
 * <i>doc-boost(d)</i> are known at indexing time. They are computed in advance
 * and their multiplication is saved as a single value in the index:
 * <i>norm(d)</i>. (In the equations below, <i>norm(t in d)</i> means
 * <i>norm(field(t) in doc d)</i>
 * where <i>field(t)</i> is the field associated with term <i>t</i>.)
 * </li>
 * </ul>
 *
 * <p>
 * <i>Lucene's Practical Scoring Function</i> is derived from the above. The
 * color codes demonstrate how it relates to those of the <i>conceptual</i>
 * formula:
 *
 * <table cellpadding="2" cellspacing="2" border="0" style="width:auto; margin-left:auto; margin-right:auto" summary="formatting only">
 * <tr><td>
 ...
 * <A NAME="formula_tf"></A>
 * <b><i>tf(t in d)</i></b>
 * correlates to the term's <i>frequency</i>, defined as the number of times
 * term <i>t</i> appears in the currently scored document <i>d</i>. Documents
 * that have more occurrences of a given term receive a higher score. Note that
 * <i>tf(t in q)</i> is assumed to be <i>1</i> and therefore it does not appear
 * in this equation, However if a query contains twice the same term, there will
 * be two term-queries with that same term and hence the computation would still
 * be correct (although not very efficient). The default computation for <i>tf(t
 * in d)</i> in
 * {@link org.apache.lucene.search.similarities.DefaultSimilarity#tf(float) DefaultSimilarity}
 * is:
 *
 * <br>&nbsp;<br>
 ...
 * (the number of documents in which the term <i>t</i> appears). This means
 * rarer terms give higher contribution to the total score.
 * <i>idf(t)</i> appears for <i>t</i> in both the query and the document, hence
 * it is squared in the equation. The default computation for <i>idf(t)</i> in
 * {@link org.apache.lucene.search.similarities.DefaultSimilarity#idf(long, long) DefaultSimilarity}
 * is:
 *
 * <br>&nbsp;<br>
 ...
 * specified document. Typically, a document that contains more of the query's
 * terms will receive a higher score than another document with fewer query
 * terms. This is a search time factor computed in
 * {@link #coord(int, int) coord(q,d)} by the Similarity in effect at search
 * time.
 * <br>&nbsp;<br>
 * </li>
 *
 ...
 * <i>queryNorm(q)</i>
 * </b>
 * is a normalizing factor used to make scores between queries comparable. This
 ...
 * different queries (or even different indexes) comparable. This is a search
 ...
 * {@link org.apache.lucene.search.BooleanQuery} computes this value as:
 *
 * <br>&nbsp;<br>
 ...
 * &nbsp; = &nbsp;
 * {@link org.apache.lucene.search.BoostQuery#getBoost() q.getBoost()}
 * <sup><big>2</big></sup>
 * &nbsp;&middot;&nbsp;
 * </td>
 * <td valign="bottom" align="center" rowspan="1" style="text-align: center">
 ...
 * <b><i>t.getBoost()</i></b>
 * is a search time boost of term <i>t</i> in the query <i>q</i> as specified in
 * the query text (see
 * <A HREF="{@docRoot}/../queryparser/org/apache/lucene/queryparser/classic/package-summary.html#Boosting_a_Term">query
 ...
 * multi {@link org.apache.lucene.search.TermQuery TermQuery} objects, and so
 * the boost of a term in the query is accessible by calling the sub-query
 ...
 * <b><i>norm(t,d)</i></b> encapsulates a few (indexing time) boost and length
 * factors:
 *
 * <ul>
 ...
 * </li>
 * <li><b>lengthNorm</b> - computed when the document is added to the index in
 ...
 * </li>
 * </ul>
 * The {@link #computeNorm} method is responsible for combining all of these
 * factors into a single float.
 *
 * <p>
 ...
 * <tr valign="top">
 * <td></td>
 * <td align="center" style="text-align: center"><small>field <i><b>f</b></i> in
 * <i>d</i> named as <i><b>t</b></i></small></td>
 * <td></td>
 * </tr>
 * </table>
 ...
 */
public abstract class TFIDFSimilarity2 extends Similarity {

    /**
     * Sole constructor. (For invocation by subclass constructors, typically
     * implicit.)
     */
    public TFIDFSimilarity2() {
    }
    ...
    public SimWeight computeWeight(CollectionStatistics collectionStats, TermStatistics... termStats) {
        ...
        return new TFIDFSimilarity2.IDFStats(collectionStats.field(), idf);
    }
    ...
    private static class IDFStats extends SimWeight {
        ...
        public void normalize(float queryNorm, float boost) {
            ...
            queryWeight = queryNorm * boost ;// modif reda
...
//        subs.add(stats.idf);
        ...
        return Explanation.match(
                boostExpl.getValue() *  queryNormExpl.getValue(),
