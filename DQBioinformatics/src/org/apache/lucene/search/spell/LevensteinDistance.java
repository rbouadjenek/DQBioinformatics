...
//        return 1.0f - ((float) p[n] / Math.max(other.length(), sa.length));
        /*
         Normalization using NORMALIZED GENERALIZED LEVENSHTEIN DISTANCE (NGLD):
         L. Yujian and L. Bo, "A Normalized Levenshtein Distance Metric," in IEEE Transactions 
         on Pattern Analysis and Machine Intelligence, vol. 29, no. 6, pp. 1091-1095, June 2007.
         doi: 10.1109/TPAMI.2007.1078
         */
        return 1 - (2 * (float) p[n]) / ((float) (other.length() + sa.length) - (float) p[n]);
    }
