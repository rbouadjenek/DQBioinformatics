...
/**
 * Class for representing a text match, containing the matched text, document
 * coordinates and normalized IDs.
 *
 * @author Martin
 *
 */
public class Mention implements Comparable<Mention>, Serializable {
    ...
    /**
     * Saves a list of mentions to a stream, in BioCreative 2 format, suitable
     * for evaluation.
     *
     * @param outStream
     * @param mentions The mentions, from a _single_ document.
     * @param restrictBySpecies if not null, any mentions not from the specified
     * species will be ignored
     * @param toSpeciesMap map from mention ids to species ids, used if
     * restrictBySpecies is not null
     */
    ...
    /**
     * If containsID(id) is true, will set ids to id only (any other ids will be
     * deleted), otherwise throws an exception
     *
     * @param id
    ...
    /**
     * @return a String on the format "id<tab>doc
     * id<tab>start<tab>end<tab>text<tab>comment" (without citation marks)
     */
    ...
    /**
     * Loads matches using loafFromFile(file,validDocumentIDs,conversionMap) and
     * then splits them up to allow access by document ID. Using the resulting
     * HashMap, the list of matches for a specific document can quickly be
     * retrieved.
     *
     * @param file
     * @param validDocumentIDs
     * @param conversionMap
     * @return a HashMap mapping document IDs to a list of matches for that
     * document.
     */
    public static Map<String, List<Mention>> loadFromFileToHash(File file, Set<String> validDocumentIDs, String restrictPostfix, HashMap<String, String> conversionMap) {
        Map<String, List<Mention>> res = new HashMap<>();
    ...
    /**
     * Will load a set of matches from a file, constrained such that only
     * matches from documents in validDocumentIDs are returned. If conversionMap
     * isn't null, matches can also be returned if
     * validDocumentIDs.contains(conversionMap.get(documentID_for_the_match)) is
     * true.
     *
     * @param file
     * @param validDocumentIDs
    ...
    /**
     * Saves the list of matches to file, sorted by their start and end
     * coordinates.
     *
     * @param matches
     * @param file
    ...
    /**
     * @return the probabilities that might be associated with the IDs (this
     * might not exist, in which case some/all might be null)
     */
    ...
    /**
     * @param probabilities an array of probabilities associated with the IDs,
     * need to be the same number as the IDs
     */
     ...
     *
     * @param pstmt Statement for inserting the match to a database, with the
     * following fields: 1: entity id, 2: document, 3: start, 4: end, 5: text,
     * 6: comment
     */
    ...
    public static List<Mention> getMentionsInRange(List<Mention> mentions,
        ...
        List<Mention> aux = new ArrayList<>();
