...
public class SimpleRankedDocRef extends AbstractChangeable implements RankedDocRef {
    ...
    /**
     * Constructs a new docref for a given location. If one or the other of
     * start and end are null, only the non-null value is used. If both are
     * null, no value is used for the location.
     *
    ...
    private void createLocation() {
        if (this.start == null && this.end == null) {
            location = RichLocation.EMPTY_LOCATION;
        } else if (this.start == null) {
            location = new SimpleRichLocation(null, new SimplePosition(this.end), 0);
        } else if (this.end == null) {
            location = new SimpleRichLocation(new SimplePosition(this.start), null, 0);
        } else {
            location = new SimpleRichLocation(new SimplePosition(this.start), new SimplePosition(this.end), 0);
        }
    }
    ...
    /**
     * {@inheritDoc} Two ranked document references are equal if they have the
     * same rank and refer to the same location and same document reference.
     */
    @Override
    ...
    /**
     * {@inheritDoc} Ranked document references are sorted first by rank then
     * location then by actual document reference.
     */
    @Override
