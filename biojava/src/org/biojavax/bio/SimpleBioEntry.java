...
/**
 * Reference implementation of a BioEntry object which has no features or
 * sequence. Equality is the combination of namespace, name, accession and
 * version.
 *
...
public class SimpleBioEntry extends AbstractChangeable implements BioEntry {
    ...
    } // protected so SimpleRichSequence can extend us

    /**
     * {@inheritDoc}
     * <b>Warning</b> this method gives access to the original Collection not a
     * copy. This is required by Hibernate. If you modify the object directly
     * the behaviour may be unpredictable.
     */
    @Override
    public Set getRankedCrossRefs() {
        return this.rankedcrossrefs;
    } // original for Hibernate
    ...
    /**
     * {@inheritDoc} Two bioentries are equal if they share the same namespace,
     * name, accession and version.
     */
    @Override
    ...
    /**
     * {@inheritDoc} Bioentries are ordered first by namespace, then name,
     * accession, and finally version.
     */
    @Override
    ...
    /**
     * {@inheritDoc} Form: namespace:name/accession.version
     */
    @Override
    ...
    public void setSource(Source source) throws ChangeVetoException {
        if (!this.hasListeners(BioEntry.SOURCE)) {
            this.source = source;
        } else {
            ChangeEvent ce = new ChangeEvent(
                    this,
            ...
            ChangeSupport cs = this.getChangeSupport(BioEntry.SOURCE);
            synchronized (cs) {
                cs.firePreChangeEvent(ce);
                this.source = source;
                cs.firePostChangeEvent(ce);
