...
/**
 * Parses a stream into sequences. This object implements SequenceIterator, so
 * you can loop over each sequence produced. It consumes a stream, and uses a
 * SequenceFormat to extract each sequence from the stream. It is assumed that
 * the stream contains sequences that can be handled by the one format, and that
 * they are not seperated other than by delimiters that the format can handle.
 ...
 * do sequence-by-sequence processing, while being guaranteed that
 * RichStreamReader will not require you to keep any of the sequences in memory.
 *
...
public class RichStreamReader implements RichSequenceIterator {
    ...
    /**
     * Creates a new stream reader on the given input stream, which will attempt
     * to read sequences in the given format, having symbols from the given
     * tokenization, and pass them to the given factory to be transformed into
     * RichSequence objects in the given namespace.
     *
     * @param is the input stream to read from
     * @param format the input file format
     * @param symParser the tokenizer that understands the sequence symbols in
     * the file
    ...
    /**
     * Creates a new stream reader on the given reader, which will attempt to
     * read sequences in the given format, having symbols from the given
     * tokenization, and pass them to the given factory to be transformed into
     * RichSequence objects in the given namespace.
     *
