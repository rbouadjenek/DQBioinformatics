...
/**
 * This class contains functions accessing DNA sequences in Genbank format. It
 * adds methods to return RichSequences instead of plain Sequences.
 *
 * @author Lei Lai
...
public class GenbankRichSequenceDB extends AbstractRichSequenceDB implements RichSequenceDBLite {
    ...
    /**
     * The default constructor delegates to the parent class. The constructor
     * refers to RichObjectFactory.getDefaultNamespace() so make sure your
     * factory is initialised before calling this constructor. Sets the default
     * factory to THRESHOLD.
     */
    ...
    /**
     * Get the URL object for locating sequence object using eutils. The default
     * value of the return format of the sequence object is text.
     *
     */
     ...
     */
    protected URL makeBatchRequest(Set list) throws MalformedURLException {
        FetchURL seqURL = new FetchURL("Genbank", "text");
        String baseurl = seqURL.getbaseURL();
        String db = seqURL.getDB();
        String id = "";
    ...
    /**
     * Given the appropriate Genbank ID, return the matching RichSequence
     * object.
     *
     * @param id the Genbank ID to retrieve.
     * @return the matching RichSequence object, or null if not found.
     * @throws Exception if the sequence could not be retrieved for reasons
     * other than the identifier not being found.
     */
     ...
     * @throws org.biojava.bio.seq.db.IllegalIDException
     */
    public void writeFastRichSequence(String id, PrintWriter out) throws BioException, IllegalIDException {
        try {
            URL queryURL = getAddress(id); //get URL based on ID
            DataInputStream in = new DataInputStream(queryURL.openStream());
            BufferedReader reader = new BufferedReader(new InputStreamReader(in));
    ...
    /**
     * Retrieve rich sequences from a Genbank. This method is written by Reda.
     *
     * @param list List of NCBI sequence number (GI), accession,
     * accession.version, fasta or seqid.
     * @return The rich database object (HashSequenceDB) with downloaded rich
     * sequences. You will need to cast the sequences you get from this database
     * object into RichSequence objects if you want to access their full
     * features.
     */
    public RichSequenceDB getRichSequences2(Set list) throws BioException, IllegalIDException {
        try {
            RichSequenceDB database = new HashRichSequenceDB();

            URL queryURL = makeBatchRequest(list); //get URL based on ID
            SymbolTokenization rParser = DNATools.getDNA().getTokenization("token"); //get SymbolTokenization
            RichSequenceBuilderFactory seqFactory = this.getFactory();
            Namespace ns = this.getNamespace();
            DataInputStream in = new DataInputStream(queryURL.openStream());
    ...
    /**
     * Given the appropriate Genbank ID, return the matching RichSequence
     * object. Additionally define a new Namespace for the received RichSequence
     * object.
     *
     * @param id the Genbank ID to retrieve.
     ...
     * @param list List of NCBI sequence number (GI), accession,
     * accession.version, fasta or seqid.
     * @param database Where to store rich sequences. If database is null, use
     * an HashSequenceDB Object.
     * @return The database object with downloaded rich sequences. You will need
     * to cast the sequences you get from this database object into RichSequence
     * objects if you want to access their full features.
     */
