...
public class CacheRecordManager
     ...
     * If entry became dirty, it is moved to _hash with limited size. This map
     * is accessed from SoftCache Disposer thread, so all access must be
     * synchronized
     */
    ...
    /**
     * End of linked-list of cache elements. Last entry is element which has
    ...
    public CacheRecordManager(RecordManager recman, int maxRecords, boolean softCache) {
        ...
        _hash = new LongHashMap<>(maxRecords);
        _recman = recman;
        _max = maxRecords;
        _softCache = softCache;

        if (softCache) {
            _softHash = new LongHashMap<>();
